...
class TestApplicationIntegration:
    ...
    def test_app_startup(self, app):
        ...
        with app.app_context():
            ...
            assert len(table_names) >= 0  # Should have some tables after db.create_all()

    def test_database_models_integration(self, app):
        """Test integration between different database models."""
        with app.app_context():
            from models import (
                db, User_Set, User_Parts, RebrickableSets, 
                RebrickableColors, RebrickablePartCategories, RebrickableThemes
            )
    ...
    def test_full_set_workflow(self, client):
        ...
        # Step 2: Add the set
        with patch('routes.set_search.db.session'), \
             patch('routes.set_search.RebrickableSets') as mock_sets:
            mock_set.id = 1
            mock_sets.query.filter_by.return_value.first.return_value = mock_set
            
            response = client.post('/add_set', data={
                'set_num': '10001-1',
                'status': 'complete'
            })
    ...
    def test_database_transaction_rollback(self, app):
        ...
        with app.app_context():
            from models import db, RebrickableSets
