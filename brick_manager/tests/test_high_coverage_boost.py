...
from brick_manager.services import rebrickable_service, cache_service, label_service
...
class TestAppModuleCoverage:
    ...
    def test_scheduled_sync_missing_parts_success(self, mock_minifig, mock_regular, mock_api, mock_token):
        """Test scheduled sync success path."""
        mock_regular.return_value = {'success': True, 'summary': {'local_missing_count': 10, 'actual_added': 2, 'actual_removed': 1}}
        mock_minifig.return_value = {'success': True, 'summary': {'local_missing_count': 5, 'actual_added': 1, 'actual_removed': 0}}
    ...
    @patch('brick_manager.app.sync_missing_parts_with_rebrickable')
    @patch('brick_manager.app.sync_missing_minifigure_parts_with_rebrickable')
    def test_scheduled_sync_missing_parts_failure(self, mock_minifig, mock_regular, mock_api, mock_token):
    ...
    def test_scheduled_sync_user_sets_success(self, mock_sync, mock_api, mock_token):
        ...
        mock_sync.return_value = {'success': True, 'summary': {'sets_added': 5, 'sets_removed': 2}}
...
class TestServicesCoverage:
    ...
    def test_rebrickable_service_coverage(self, mock_get):
        ...
        from brick_manager.services.rebrickable_service import make_request, get_user_sets
    ...
    def test_label_service_coverage(self, mock_font, mock_draw, mock_image):
        ...
        from brick_manager.services.label_service import generate_qr_code, create_label_pdf
...
class TestRoutesCoverage:
    ...
    def test_dashboard_routes(self):
        ...
        with app_module.app.test_client() as client:
            ...
            response = client.get('/')
            assert response.status_code in [200, 302, 404, 500]  # Any valid HTTP response
...
class TestConfigCoverage:
    ...
    def test_config_attributes(self):
        ...
        assert hasattr(config, 'SQLALCHEMY_DATABASE_URI') or hasattr(Config, 'SQLALCHEMY_DATABASE_URI')
        assert hasattr(config, 'SECRET_KEY') or hasattr(Config, 'SECRET_KEY')
        assert hasattr(config, 'REBRICKABLE_TOKEN') or hasattr(Config, 'REBRICKABLE_TOKEN')
...
class TestModelsCoverage:
    ...
    def test_model_imports(self):
        ...
        from brick_manager.models import (
            Parts, Sets, Storage, PartLookup, Colors, 
            RebrickableSets, RebrickableParts, MissingParts,
            UserSets, Themes, Categories, db
        )
