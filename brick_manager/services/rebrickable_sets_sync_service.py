...
def get_brick_manager_list():
    ...
    try:
        ...
        if brick_manager_list:
            logger.info(
                f"Found existing Brick_Manager list with ID: {brick_manager_list.get('id')}"
            )
            return {
                "success": True,
    ...
    try:
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to get list sets: {response.status_code} - {response.text}"
                )
                break
    ...
    try:
        ...
        if not sets_to_process:
            return {
                ...
                "message": "No sets to add",
            }

        logger.info(
            f"Adding {len(sets_to_process)} sets to Rebrickable list using bulk API"
        )
        ...
        for set_num, quantity in sets_to_process:
            bulk_data.append({"set_num": set_num, "quantity": quantity})
        ...
        try:
            ...
            logger.warning(
                f"Bulk set addition error, falling back to individual requests: {e}"
            )
    ...
    for i, (set_num, quantity) in enumerate(sets_to_process):
        try:
            ...
            if response.status_code == 201:
                ...
                error_msg = (
                    f"Failed to add {set_num} (qty: {quantity}): {response.text}"
                )
...
def update_set_quantity_in_list(list_id, set_num, new_quantity):
    ...
    try:
        ...
        if delete_response.status_code not in [
            204,
            404,
        ]:  # 404 is okay if set wasn't in list
            logger.warning(
                f"Failed to remove set {set_num} for update: {delete_response.status_code} - {delete_response.text}"
            )
            return {
                "success": False,
...
def sync_user_sets_with_rebrickable():
    ...
    try:
        ...
        logger.info(
            f"Found {len(local_set_quantities)} unique local user sets with total quantity {sum(local_set_quantities.values())}"
        )
        ...
        logger.info(
            f"Found {len(rebrickable_set_quantities)} sets in Rebrickable list with total quantity {sum(rebrickable_set_quantities.values())}"
        )
        ...
        sets_to_add = (
            local_set_nums - rebrickable_set_nums
        )  # Sets not in Rebrickable at all
        sets_to_remove = (
            rebrickable_set_nums - local_set_nums
        )  # Sets in Rebrickable but not local
        ...
        logger.info(
            f"Sets to add: {len(sets_to_add)}, Sets to remove: {len(sets_to_remove)}, Sets to update quantities: {len(sets_to_update)}"
        )
        ...
        if sets_to_add:
            ...
            sets_to_add_with_qty = {
                set_num: local_set_quantities[set_num] for set_num in sets_to_add
            }
            add_result = add_sets_to_list(list_id, sets_to_add_with_qty)
            results["operations"].append(
                {"operation": "add_sets", "result": add_result}
            )
        ...
        if sets_to_update:
            for set_num, local_qty, rebrickable_qty in sets_to_update:
                ...
                if update_result.get("success"):
                    updated_count += 1
                    logger.debug(
                        f"Updated {set_num} quantity from {rebrickable_qty} to {local_qty}"
                    )
                    ...
                    logger.warning(
                        f"Failed to update {set_num} quantity: {update_result.get('message')}"
                    )
        ...
        total_added = sum(
            op["result"].get("added_count", 0)
            for op in results["operations"]
            if op["operation"] == "add_sets"
        )
        total_removed = sum(
            op["result"].get("removed_count", 0)
            for op in results["operations"]
            if op["operation"] == "remove_sets"
        )
        total_updated = sum(
            op["result"].get("updated_count", 0)
            for op in results["operations"]
            if op["operation"] == "update_quantities"
        )
        total_rate_limited = sum(
            op["result"].get("rate_limited_count", 0)
            for op in results["operations"]
            if op["operation"] == "add_sets"
        ) + sum(
            op["result"].get("rate_limited_count", 0)
        ...
        if total_rate_limited > 0:
            message_parts.append(
                f" {total_rate_limited} operations were rate limited and will be synced automatically in the next scheduled run."
            )

        message_parts.append(
            f" Total unique sets in local collection: {len(local_set_quantities)} (total quantity: {sum(local_set_quantities.values())})"
        )

        results["summary"]["message"] = "".join(message_parts)

        logger.info(
            f"User sets synchronization completed - added {total_added}, updated {total_updated}, removed {total_removed}"
        )
