...
def get_category_name_from_db(part_cat_id):
    ...
    try:
        category = RebrickablePartCategories.query.filter_by(
            id=part_cat_id).first()

        if category:
            logging.debug(
                "Successfully found category: %s for part_cat_id: %s", category.name, part_cat_id)
            return category.name
        else:
            logging.warning(
                "No category found for part_cat_id: %s", part_cat_id)
            return 'Unknown Category'

    except Exception as e:
        logging.error(
            "Database error while fetching category for part_cat_id %s: %s", part_cat_id, e)
        return 'Unknown Category'
...
def get_category_name_from_part_num(part_num):
    ...
    try:
        ...
        logging.debug(
            "Querying RebrickableParts table for part_num: %s", part_num)
        part_info = RebrickableParts.query.filter_by(part_num=part_num).first()

        if not part_info:
            logging.warning(
                "No part found in RebrickableParts for part_num: %s", part_num)
            return 'Unknown Category'
        ...
        logging.debug("Found part_cat_id: %s for part_num: %s",
                      part_info.part_cat_id, part_num)

        # Query RebrickablePartCategories for the part_cat_id
        logging.debug(
            "Querying RebrickablePartCategories table for part_cat_id: %s", part_info.part_cat_id)
        category = RebrickablePartCategories.query.filter_by(
            id=part_info.part_cat_id).first()
