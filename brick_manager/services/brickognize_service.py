...
def get_predictions(file_path, filename):
    ...
    """
    logging.info("Starting Brickognize prediction for file: %s (path: %s)", filename, file_path)
    ...
    try:
        ...
        logging.info("Received response from Brickognize API - Status Code: %s", response.status_code)
        ...
        logging.error("Request to Brickognize API failed for file %s: %s", filename, req_err)
        logging.error("Request details - URL: %s, Timeout: 10s", api_url)
        return None
    except ValueError as json_err:
        logging.error("Error decoding JSON response from the Brickognize API for file %s: %s", filename, json_err)
        logging.error("Response content (first 500 chars): %s", response.text[:500] if 'response' in locals() else 'No response available')
        return None
    except FileNotFoundError as file_err:
        logging.error("File not found: %s - %s", file_path, file_err)
        return None
    except Exception as e:
        logging.error("Unexpected error during Brickognize API call for file %s: %s", filename, e)
        return None
    ...
    if predictions and 'items' in predictions:
        ...
        for idx, item in enumerate(predictions['items']):
            ...
            logging.debug("Processing item %d/%d: part_num=%s", idx + 1, items_count, part_num)
            
            if part_num:
                try:
                    category_name = get_category_name_from_part_num(part_num)
                    item['category_name'] = category_name
                    logging.debug("Successfully enriched part_num %s with category: %s", part_num, category_name)
