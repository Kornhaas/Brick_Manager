...
from models import ConfigSettings, db
...
def generate_token():
    ...
    try:
        ...
        api_key_config = ConfigSettings.query.filter_by(
        ...
        if response.status_code in [200, 201]:
            ...
            if user_token:
                ...
                logger.info(
                    f"Successfully generated and stored Rebrickable token for user: {username}"
                )
                return jsonify(
                    {
                        "success": True,
        ...
        return jsonify(
            {"success": False, "message": "Timeout connecting to Rebrickable API"}
        )
...
def test_token():
    ...
    try:
        ...
        if not api_key_config:
            return jsonify(
                {"success": False, "message": "Rebrickable API key not configured"}
            )
