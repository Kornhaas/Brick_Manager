        ...
        if not set_number:
            ...
            for minifig in minifigs_info:
                ...
                if fig_num:
                    ...
                    current_app.logger.debug(
                        "Fetched %d parts for minifigure %s.",
                        len(minifig_parts),
                        fig_num,
                    )
...
def add_set():
    ...
    try:
        ...
        for part in parts_info:
            ...
            # Get or create color
            color_info, _ = get_or_create(
                db.session,
                RebrickableColors,
                name=part["color"],
                defaults={"rgb": part["color_rgb"], "is_trans": False},
            )
        ...
        flash(
            f"Set {template_set.name} added successfully as {status_description}!",
            category="success",
        )
...
def fetch_set_parts_info(set_number):
    ...
    try:
        ...
        inventory_parts = (
            db.session.query(
                RebrickableInventoryParts, RebrickableParts, RebrickableColors
            )
            .join(
                RebrickableParts,
                RebrickableInventoryParts.part_num == RebrickableParts.part_num,
            )
            .join(
                RebrickableColors,
                RebrickableInventoryParts.color_id == RebrickableColors.id,
            )
            .filter(RebrickableInventoryParts.inventory_id == inventory.id)
            .all()
        )
        ...
        current_app.logger.debug(
            "Fetched %d parts for set %s from local database",
            len(parts_info),
            set_number,
        )
        return parts_info
...
def fetch_minifigs_info(set_number):
    ...
    try:
        ...
        inventory_minifigs = (
            db.session.query(RebrickableInventoryMinifigs, RebrickableMinifigs)
            .join(
                RebrickableMinifigs,
                RebrickableInventoryMinifigs.fig_num == RebrickableMinifigs.fig_num,
            )
            .filter(RebrickableInventoryMinifigs.inventory_id == inventory.id)
            .all()
        )
        ...
        current_app.logger.debug(
            "Fetched %d minifigs for set %s from local database",
            len(minifigs_info),
            set_number,
        )
...
def fetch_minifigure_parts(fig_num):
    ...
    try:
        ...
        current_app.logger.debug(
            "Fetched %d parts for minifigure %s from local database",
            len(parts_info),
            fig_num,
        )
        return parts_info
