...
from models import db, User_Set, User_Parts, UserMinifigurePart, PartStorage, RebrickableParts, RebrickableColors, RebrickableInventories, RebrickableInventoryParts
            ...
            for part in user_set.parts_in_set:
                if part.quantity > part.have_quantity:
                    ...
                    if part_category == decoded_category_name:
                        try:
                            enriched_part = enrich_missing_part(
                                part, user_set, 'Regular Part')
                            missing_items.append(enriched_part)
                        except Exception as e:
                            current_app.logger.error(
                                f"Error enriching regular part {part.part_num}: {str(e)}")
                            continue
            ...
            if minifig_part.quantity > minifig_part.have_quantity:
                ...
                if part_category == decoded_category_name:
                    try:
                        ...
                        current_app.logger.error(
                            f"Error enriching minifigure part {minifig_part.part_num}: {str(e)}")
                        continue
