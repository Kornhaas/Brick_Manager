    ...
    try:
        ...
        for user_set in user_sets:
            ...
            if set_num_base in available_instructions:
                ...
                theme_name = template.theme.name if template and template.theme else None
        ...
        logger.info(f"Found instructions for {len(sets_with_instructions)} out of {len(user_sets)} sets")
...
def download_instruction_file(file_path):
    ...
    try:
        ...
        if not os.path.abspath(full_path).startswith(os.path.abspath(instructions_folder)):
            logger.warning(f"Attempted access to file outside instructions folder: {file_path}")
            abort(403)
